...
def try_download_detailed_provinces() -> gpd.GeoDataFrame:
    """
    Try to download detailed Natural Earth provinces data, with 365-day caching.
    ...
    def fetch():
                ...
                if url.endswith('.geojson'):
                    ...
                    with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        with zipfile.ZipFile(zip_path, 'r') as zf:
                            ...
                            world = gpd.read_file(os.path.join(extract_dir, shp_files[0]))
                            canada = world[world['admin'] == 'Canada'].copy()
                            if len(canada) > 0:
                                canada = canada.to_crs('EPSG:3978')
                                print(f"Downloaded {len(canada)} provinces/territories from shapefile")
                                return canada
        ...
        # All URLs failed — use embedded fallback
        print("Falling back to embedded province data...")
        return load_provinces_geojson()

    return cache.get_or_fetch('provinces', fetch)
